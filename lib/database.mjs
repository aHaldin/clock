...
let db;
export function getConfig(){return {...process.env,PH_BUILD_ORIGINS:process.env.PH_BUILD_ORIGINS,ADMIN_USER_IDS:'password-admin',ANY_DEVICE_CLOCKING:process.env.ANY_DEVICE_CLOCKING??'true',LOCATION_REQUIRED:'true',BROWSER_CLOCKING:process.env.BROWSER_CLOCKING??'true',LOCAL_CLOCKING:process.env.NODE_ENV==='development'&&process.env.LOCAL_CLOCKING==='true'}}
