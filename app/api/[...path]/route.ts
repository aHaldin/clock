...
import {prepareAnyDevice} from '@/lib/any-device.mjs';
...
if(config.ANY_DEVICE_CLOCKING==='true'&&(path==='device'||path==='clock/verify')){const prepared=await prepareAnyDevice(req,config);req=prepared.req;if(prepared.cookie)headers.append('Set-Cookie',prepared.cookie);}
let data:any;
...
else if(path==='clock/reset'&&req.method==='POST'){const token=req.headers.get('cookie')?.split(';').map(x=>x.trim()).find(x=>x.startsWith('ph_session='))?.slice(11);if(token)await service.stmt('DELETE FROM sessions WHERE token_hash=?',await sha(token)).run();headers.append('Set-Cookie',cookie(''));data={ok:true};}
else if(path.startsWith('clock/')&&req.method==='POST'){const result=await service.clock(req,path.slice(6),body,raw);if(result.token)headers.append('Set-Cookie',cookie(result.token));if(result.clear)headers.append('Set-Cookie',cookie(''));data=result.data;}
